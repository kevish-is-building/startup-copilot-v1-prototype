...
import { getCachedRecommendations, setCachedRecommendations, isRedisConfigured } from '@/lib/cache';
import { buildPersonalizationPrompt } from '@/lib/prompts';
import type { UserProfile, AIRecommendationResponse, ContentRecommendation, PromptCache } from '@/lib/types/ai';
import { Ratelimit } from '@upstash/ratelimit';
import { Redis } from '@upstash/redis';

// Rate limiter setup (only if Redis is configured)
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (limiter) {
      const { success, remaining } = await limiter.limit(`recommendations:${userId}`);
      if (!success) {
        return NextResponse.json(
    ...
    if (isRedisConfigured()) {
      const cached = await getCachedRecommendations(userProfile);
    ...
    if (!isOpenAIConfigured()) {
      // Return fallback recommendations if OpenAI is not configured
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are an expert startup advisor. Always return valid JSON only.',
        },
        {
          role: 'user',
          content: prompt,
    ...
    if (isRedisConfigured()) {
      ...
      await setCachedRecommendations(userProfile, cache);
    }
    ...
    const body = await request.json().catch(() => ({ userProfile: null }));
    if (body.userProfile) {
